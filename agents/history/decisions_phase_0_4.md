...
> This archive contains the full text of D001-D038, retained for targeted lookup only. The live decision index is the default entry point.
...
## D031 - Automated Gate 2 judge model policy

**Date:** 2026-07-09 · **Status:** User approved

C003's Gate 2 design is accepted with these resolutions:

1. **Grok 4.5 is the default** handoff executor and automated Gate 2 judge
   model. A handoff may instead name another approved high-reasoning roster model
   when its risk and reasoning need justify that spend and the user approves the
   dispatch. The exact model used must be recorded with the stored result.
2. The judge is used only as a validation-time "strong player"
...
3. D022's "pinned model at temperature 0" wording is clarified to mean: pin the
   **selected handoff model** and prompt version for each stored run; use the
   most deterministic configuration that model supports, with temperature
...
**Rationale:** Grok 4.5 remains cost-effective for routine validation, while
exceptionally difficult identity judgments can merit a stronger approved model.
Recording the exact model and prompt keeps every stored result falsifiable and
reproducible. This does not weaken the `soul.md` ban on dynamic AI-generated
production gameplay because the model judges already-authored cards offline; it
does not create served content.
...
## D039 - Diff-summary review gate

**Date:** 2026-07-09 - **Status:** User approved

Routine low- and medium-risk report review uses the automated DeepSeek v4 Pro Diff
Summarizer before the orchestrator examines a raw diff. The summarizer reads the
completion report and scoped uncommitted diff, then writes a 200-word-or-less
verification summary. The orchestrator reviews that summary, the report, and cheap
verification output; it opens raw diff only for a discrepancy, failed check,
high-risk work, or a material acceptance-criterion question. High-risk reviews
retain their required strong-model/formal review depth under D024.

**Rationale:** Raw code diffs are syntax-dense and costly for the high-reasoning
orchestrator. A constrained low-cost first pass preserves evidence while keeping
routine approval context small.

## D040 - Phase-boundary archive and compact-evidence policy

**Date:** 2026-07-09 - **Status:** User approved

At a phase close or major milestone, the orchestrator archives accepted handoffs,
reports, audits, consultations, and no-longer-live machine artifacts into a named
phase bundle under `agents/history/`. Active `agents/handoffs`, `agents/reports`,
and `agents/gate2` retain only current/next work, awaiting-review reports, and
artifacts required by a live handoff. `progress.md` remains the compact live
dashboard; the default reader sees Current Status, How to Run, Blocked/Questions,
and only the latest session entry.

Completion reports default to 400 words or fewer plus their acceptance table.
Structured evidence belongs in a named machine-readable artifact and is summarized
in the report, not duplicated there. Handoffs use proportionate verification:
content-only data changes run focused validation/tests, while root tests/typecheck
remain for source/schema/configuration changes, explicit cross-package risks, and
phase gates. Existing executed handoffs are not rewritten; the rules apply to new
work after the decision.

**Rationale:** Context manifests already prevent deliberate over-reading, but large
active folders and repeated evidence create accidental context load and expensive
orchestrator review. This keeps history discoverable without making it default
context, while retaining the safeguards that protect game rules, content integrity,
and uncommitted-work review.

## D041 - Boot-first interactive orchestrator startup

**Date:** 2026-07-09 - **Status:** User approved

When an interactive session begins without an assigned role or handoff, the agent
is the Orchestrator and reads `agents/orchestrator_boot.md` before any other
project document. The boot card directs the minimal startup reads. The full
`agents/roles/orchestrator.md` is an on-demand workflow reference, read only when
the agent needs to author a handoff, review a report, record a decision, or commit
approved work.

**Rationale:** The boot card exists to avoid loading the full orchestrator role and
routing policy merely to orient a fresh session. Making it the explicit entry point
removes that ambiguity while preserving detailed guidance at the point of action.

## D042 - Browser-required Playtester role

**Date:** 2026-07-09 - **Status:** User approved

Create a permanent, report-only Playtester role for human-perspective evaluation
of Signal or Noise?. It is invoked only when the user or orchestrator requests a
playtest; it is not a routine gate. A valid playtest runs in an interactive native
desktop session with browser control: Codex's in-app Browser or Claude Desktop's
built-in browser. Headless `codex exec` and `claude -p` sessions do not qualify.

The role self-blinds before a decision by avoiding source, scenario JSON, reveal,
and outcome data. It records observed behavior separately from simulated-player
inferences, never changes code/content/rules, and cannot replace external human
testing, blind Gate 2, human content review, accessibility verification, or an
Auditor. Its report is `agents/playtests/P###_H###.md`.

**Rationale:** Browser evidence makes AI feedback useful for real player journeys
without presenting simulated feedback as human research. The role has recurring,
distinct ownership across gameplay UI and content calibration, while final product
decisions and follow-up implementation remain with the orchestrator and existing
roles.

## D043 - Phase-oriented autonomous workflow

**Date:** 2026-07-09 - **Status:** User approved

The roadmap phase is now the default unit of agent execution. The user approves
one concise phase charter; one high-autonomy Phase Owner discovers, implements,
tests, and repairs the phase end to end; and a different capable model reviews the
complete phase once at its boundary. Internal batches, validators, blind judging,
and checkpoint commits do not create handoffs, reports, approval gates, or
orchestrator reviews.

New normal work uses `agents/phases/` and one closeout under
`agents/phase-closeouts/`. H###/R### chains, per-session progress logs, mandatory
diff summaries, manual-per-handoff dispatch, and fix-up handoffs are retired.
Existing artifacts remain historical or active-WIP evidence. `progress.md` is
current state only.

After the user authorizes a phase, the Orchestrator may dispatch directly in the
user's selected harness. Phase Owners may make reversible in-scope decisions and
repair, retire, or replace failed artifacts without escalation. Stop conditions
are limited to decisions outside delegated authority, locked-rule conflicts,
required external access/spend, irreversible or outward-facing actions, and true
acceptance blockers.

A dedicated non-main branch is recommended. A separate Git worktree is optional
and used for parallelism, dirty-checkout isolation, or when the harness manages it
automatically. Phase Owners may make checkpoint commits only on a dedicated phase
branch and never push without approval. Claude Code, Codex, and T3 Code are all
supported; repository policy remains harness-independent.

This decision supersedes the operational workflow in D012, D024, D028-D030,
D039-D042 where they conflict. Product/content safeguards remain: phase owners
self-test continuously, high-risk phases receive independent end review, blind
content judging remains independent validation, and human activation/release
authority remains at the phase boundary.

**Rationale:** The prior system reduced context size but multiplied context reloads,
prompts, reports, approvals, and reviews. Phase ownership retains scoped startup
and compact evidence while spending tokens on implementation and one meaningful
review rather than coordination artifacts.

## D044 - Relax Medium/Hard Gate 2 to dominance thresholds

**Date:** 2026-07-09 - **Status:** User approved

Player testing found the previous Medium/Hard anti-identification thresholds made
cards excessively abstract. Gate 2 now permits the correct company to be a
plausible or even leading strong-player guess and fails only near-certain
dominance:

- Medium: correct is #1 at >=85% confidence **and** leads #2 by >=35 points.
- Hard: correct is #1 at >=75% confidence **and** leads #2 by >=35 points.
- Easy is unchanged: the answer should remain attainable in the top three.

Direction and plausible-count findings remain WARN-only. Raw pinned judge results
stay unchanged; thresholds are evaluated locally, allowing calibration without
spending another model pass.

**Rationale:** Hidden-company ambiguity is a means to fair gameplay, not an end in
itself. Recognizable clues give players something to reason about; only dominant,
near-certain identification undermines Call the Company and difficulty integrity.

## D045 - Phase 4B acceptance and production activation

**Date:** 2026-07-09 - **Status:** User approved

The user accepted the D038 human source/content gate for the reviewed 40-card
Phase 4B set. Promote every approved card to `active` with `humanReviewed: true`,
wire the active content bundle into Classic Run, and archive the six prototype
seeds outside gameplay rather than deleting them.

**Rationale:** The phase is intended to replace prototype-grade playable content
with a complete, reviewed production set while retaining the old seeds only as
historical fixtures.
