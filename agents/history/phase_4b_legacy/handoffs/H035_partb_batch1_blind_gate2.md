...
**Output budget:** report <= 400 words plus acceptance table; full per-variant
results in `agents/gate2/H035_results.json`
...
Write `agents/gate2/H035_results.json` with one row per in-scope payload:
`scenarioId`, `difficulty`, `payloadHash`, top guess, correct-rank (only after
write-back), outcome, and findings. This is the detailed review evidence; keep
the completion report to aggregate counts plus any identity failures.
...
4. Content `validate`, `gate2 check`, and focused content tests run and are
   reported; `H035_results.json` provides the per-variant evidence.
...
Include: in-scope payload count; aggregate pass/fail/WARN counts; Netflix fold-in
outcome; `H035_results.json` path; and recommended next step (fix-up handoff vs
user playtest vs batch 2). List individual identity failures only; do not duplicate
all per-variant rows from the evidence artifact.
