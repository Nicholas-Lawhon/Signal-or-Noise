...
2. Approval gate is risk-based (D023): low-risk handoffs dispatch as soon as the
   user agrees to the task; medium/high-risk handoffs need explicit user
...
5. Orchestrator reviews the report + uncommitted diff, and approves or rejects.
   Medium/high-risk handoffs (per the Risk header) also get an Auditor pass
   → agents/audits/A###_H###.md — always a different model than the implementor.
