...
describe('privacy-bounded analytics adapter', () => {
  ...
  it('keeps competitive lifecycle events free of identifiers and game truth', () => { const send = vi.fn(); const adapter = createAnalyticsAdapter({ enabled: true, configured: true, send }); adapter.capture({ name: 'draft_completed', properties: {} }); adapter.capture({ name: 'battle_completed', properties: { outcome: 'draw' } }); expect(send.mock.calls).toEqual([['draft_completed', {}], ['battle_completed', { outcome: 'draw' }]]); });
});
