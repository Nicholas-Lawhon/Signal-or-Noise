...
export async function GET(
  ...
  try {
    ...
    if (!identity.user) return jsonError(401, 'UNAUTHENTICATED', 'Sign in to view this Draft Battle invite');
    const invite = await getDraftBattleService().getInvitePreview({ userId: identity.user.userId, inviteCode: code.data });
