...
function redistributeAllocation(current: number[], changedIndex: number, changedValue: number): number[] {
  const next = [...current];
  ...
  for (const [position, index] of otherIndexes.entries()) {
    const left = otherIndexes.length - position - 1;
    const value = left === 0
      ? remaining
      : Math.min(60, Math.max(10, Math.round(remaining / (left + 1) / 10) * 10));
    next[index] = value;
    remaining -= value;
  }
  return remaining === 0 && next.every((value) => value >= 10 && value <= 60) ? next : current;
}
