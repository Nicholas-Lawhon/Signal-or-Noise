/**
 * Pure, synchronous, browser-safe SHA-256 (FIPS 180-4).
 * No Node builtins — safe to import from package-root / web bundle paths.
 */
...
function utf8Bytes(text: string): Uint8Array {
  // TextEncoder is available in modern browsers and Node; avoids node: imports.
  if (typeof TextEncoder !== 'undefined') {
    return new TextEncoder().encode(text);
  }
  // Minimal UTF-8 fallback for exotic runtimes without TextEncoder.
  const out: number[] = [];
...
export function sha256Hex(message: string): string {
  ...
  if (padLen > 56) padLen = 64 - (padLen - 56);
  else padLen = 56 - padLen;
  // padLen is number of zero bytes after 0x80 before length
  // Actually: total length ≡ 56 (mod 64) before the 8-byte length.
  const totalLen = msg.length + 1 + padLen + 8;
  const block = new Uint8Array(totalLen);
  block.set(msg);
  block[msg.length] = 0x80;
  // big-endian bit length in last 8 bytes (we only need low 32 for short msgs)
